...
with musicbrainz_db.engine.begin() as connection:
    artist_query = text("""SELECT artist.id, artist.gid, artist.name, artist.sort_name,
                ...
                ON artist_credit.id = artist.id where artist_credit.id=:MB_artist_credit_data[0]
    """)
    result = connection.execute(query, {"MB_artist_credit_data": MB_artist_credit_data})
